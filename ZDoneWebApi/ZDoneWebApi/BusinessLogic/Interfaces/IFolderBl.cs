...
namespace ZDoneWebApi.BusinessLogic.Interfaces
{
    public interface IFolderBl
    {
        ...
        Task<IEnumerable<FolderDto>> GetByProjectIdAsync(int id, string userId);

        Task<IEnumerable<ListDto>> GetRelatedLists(int id, string userId);

        Task<FolderDto> ReadAsync(int id);

        Task<ItemResponse> CreateAsync(FolderDto folder);

        Task<ItemResponse> UpdateAsync(FolderDto folder, string userId);
